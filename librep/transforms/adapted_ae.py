...
class AdaptedAEDimensionalityReduction(Transform):

    def __init__(self,
                 ae_model='ConvolutionalAutoencoder', ae_kwargs=None, lam=1., metric='coknns',
                 patience=10, num_epochs=1000, batch_size=128, input_shape = (-1, 1, 28, 28)):
        self.patience = patience
        self.num_epochs = num_epochs
        self.model = MetricsRegularizedAutoencoder(
    ...
    def fit(self, X: ArrayLike, y: ArrayLike = None, title_plot=None):
        ...
        for epoch in range(self.num_epochs):
            epoch_train_loss = []
            epoch_train_ae_loss = []
            epoch_train_metric_error = []
            epoch_val_loss = []
            epoch_val_ae_loss = []
            epoch_val_metric_error = []
            self.model.train()
            for data in train_data_loader:
                ...
                epoch_train_metric_error.append(loss_components['loss.metric_error'].item())
            ...
            metric_loss_per_epoch = np.mean(epoch_val_metric_error)
            
            print(f'Epoch:{epoch+1}, P:{patience}, Loss:{loss_per_epoch:.4f}, Loss-ae:{ae_loss_per_epoch:.4f}, Loss-metric:{metric_loss_per_epoch:.4f}')
        ...
        ax2=ax.twinx()
        ax2.plot(plot_train_metric, label='Metric error - train', color='blue')
        ax2.plot(plot_val_metric, label='Metric error - val', color='black')
        ax2.set_ylabel("Metric error", color="blue",fontsize=14)
        ax2.legend(loc=1)
