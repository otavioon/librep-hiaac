...
class UMAPEmbedding_DataSubSet(Embedding_DataSubSet): 
...
class UMAPEmbedding_DataSet(Embedding_DataSet): 
    def __init__(self, flattenable_ds_class = RawAccGyrEmbedding_DataSet, n_components = 2):
        ...
        self.embedding_name = "UMAPEmbedding-{}comp-empty".format(self.n_components)
    ...
    def import_from_canonical(self, dss: Canonical_DataSet):
        ...
        self.embedding_name = "UMAPEmbedding-{}comp-{}".format(self.n_components,ft_ds.embedding_name)
        ...
        trans =  self.umap_model.fit(X, y=y)
        
        # Apply the umap predictor to the dataset
        self.test       = UMAPEmbedding_DataSubSet(trans.transform(ft_ds.test.get_X()), 
                                                   ft_ds.test.get_y(),self.embedding_name)
        self.validation = UMAPEmbedding_DataSubSet(trans.transform(ft_ds.validation.get_X()), 
                                                   ft_ds.validation.get_y(),self.embedding_name)
        self.train      = UMAPEmbedding_DataSubSet(trans.transform(ft_ds.train.get_X()), 
