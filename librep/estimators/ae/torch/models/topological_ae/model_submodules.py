...
class ConvolutionalAutoencoder_custom_dim(AutoencoderModel):
    """Convolutional Autoencoder with 2d latent space.
...
class DeepAEv2(AutoencoderModel):
    """200 - 100 - 50 - n - 50 - 100 - 200"""
    def __init__(self, input_dims=(1, 180), custom_dim=2):
        print('DeepAEv2, Input:', input_dims,'Inner dim:', custom_dim)
        super().__init__()
        self.input_dims = input_dims
        n_input_dims = np.prod(input_dims)
        self.encoder = nn.Sequential(
            View((-1, n_input_dims)),
            nn.Linear(n_input_dims, 500),
            nn.ReLU(True),
            nn.BatchNorm1d(500),
            nn.Linear(500, 250),
            nn.ReLU(True),
...
class DeepAE_custom_dim(AutoencoderModel):
