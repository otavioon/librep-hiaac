...
combination_umap = data_umap_name.copy()
combination_umap[0].insert(0, ('-',))

tam = len(data_train_name)
combination_train = data_train_name.copy()

combination_test = data_test_name.copy()
...
for set_umap in combination_umap:
    if set_umap == '-':
        for set_train in combination_train:
            combinations_sets['Umap'].append(set_umap)
            combinations_sets['Train'].append(set_train)
            combinations_sets['Test'].append(set_train) 
    else:
        combinations_sets['Umap'].append(set_umap)
        combinations_sets['Train'].append(set_umap)
        combinations_sets['Test'].append(set_umap)
...
# with open('df_results_base.pkl', 'rb') as f:
#     model = pickle.load(f)
# df_results

# with open('results_dict_base.pkl', 'rb') as f:
#     model = pickle.load(f)
# results_dict